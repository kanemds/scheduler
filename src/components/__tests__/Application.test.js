...
import { waitForElement, render, cleanup, fireEvent, queryByText, queryByAltText } from "@testing-library/react";
...
describe("Application", () => {
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async() => {
    ...
    fireEvent.change(getByPlaceholderText(appointment, /enter student name/i), {
      target: { value: "Lydia James" }
    });
    
    fireEvent.click(getByText(appointment, "Save"));
   
    await waitForElement(() => queryByText(appointment, "Lydia James"));
  ...
  it("shows the save error when failing to save an appointment", async () => {
    axios.put.mockRejectedValueOnce(Promise.resolve({
      status: 500,
      statusText:"error"
    }));
    const { container } = render(<Application />);
  ...
  it("shows the delete error when failing to delete an existing appointment", async () => {
    axios.delete.mockRejectedValueOnce(Promise.resolve({
      status: 500,
      statusText:"error"
    }));
    // 1. Render the Application.
